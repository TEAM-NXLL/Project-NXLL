...
export async function renderProductTransacs(transacs) {
  const products = document.querySelectorAll('.product-item')
  products.forEach(el => {
    const productTransacBtn = el.querySelector('.each-transac-btn-down');
    if(!productTransacBtn){
      return
    }
    
    const hideBtn = el.querySelector('.each-transac-btn-up');
    const productTransacCont = el.querySelector('.transac-container');
    ...
    productTransacBtn.addEventListener('click', (event) => {
      products.forEach(el => {
        ...
        event.path[1].classList.remove('hide')
      })
